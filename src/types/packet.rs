...
impl TcpFlags {
    ...
    /// Check if only SYN flag is set (no other flags)
    pub fn is_syn_only(&self) -> bool {
        self.syn && !self.ack && !self.fin && !self.rst && !self.psh && !self.urg
    }

    /// Check if XMAS scan flags are set (FIN+PSH+URG)
    pub fn is_xmas(&self) -> bool {
        self.fin && self.psh && self.urg
    }

    /// Check if all flags are unset (NULL scan)
...
impl Packet {
    ...
    pub fn from_ethernet_bytes(packet_id: u64, data: &[u8], interface: &str) -> Option<Self> {
        ...
        let (src_ip, dst_ip, protocol) = match &sliced.net {
            ...
            Some(etherparse::InternetSlice::Arp(_)) => return None, // ARP packets not supported
            None => return None, // Other non-IP packets
        };
    ...
    /// Get timestamp as nanoseconds since Unix epoch
...
impl Default for Packet {
    fn default() -> Self {
        ...
        Self {
            ...
            layer4: Layer4::Unknown { protocol: 0 },
            tls: None,
            flow_id: None,
            direction: Direction::Unknown,
